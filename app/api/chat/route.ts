...
async function withTimeout<T>(promise: Promise<T>, timeoutMs: number, fallback: T): Promise<T> { return Promise.race([promise, new Promise<T>(resolve => setTimeout(() => resolve(fallback), timeoutMs))]) }
async function nextWithTimeout<T>(iterator: AsyncIterator<T>, timeoutMs: number): Promise<IteratorResult<T>> { return Promise.race([iterator.next(), new Promise<IteratorResult<T>>((_, reject) => setTimeout(() => reject(new Error('Model response timed out.')), timeoutMs))]) }
async function generateAiTitle(model: any, chatId: string, userId: string, prompt: string): Promise<string | null> {
  ...
  let title = ''
  try { for await (const chunk of generateResponse(model, [titleMessage], 'Generate a short chat title from the user request. Return only 2 to 6 words, no quotes, no punctuation at the end, no explanation.', 0.2)) { title += chunk; if (title.length >= 80) break } } catch { return null }
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        try {
          ...
          const selectedModel = selectModel(enhancedMessages, Boolean(hasImages), Boolean(hasDocuments), estimateTokens(enhancedMessages.map(m => m.content || '').join(' ')))
          ...
          let success = false, fullResponse = '', respondingModel = selectedModel
          const tryModel = async (model: typeof selectedModel) => {
            ...
            try { while (true) { const result = await nextWithTimeout(iterator, 45000); if (result.done) break; candidate += String(result.value || ''); if (result.value) send({ status: 'Answering…', chunk: result.value, model: model.name }) } return candidate }
            catch (error) { try { await iterator.return?.() } catch {}; console.error(`[CarbonAI] Model ${model.id} failed:`, error); markModelUnhealthy(model.id, error instanceof Error ? error.message : 'unknown error'); throw error }
          }
          ...
          for (let index = 0; index < attempts.length && !success; index++) {
            ...
            try {
              ...
              fullResponse = candidate; respondingModel = model; success = true
            ...
            } catch (error) { if (index === attempts.length - 1) throw error }
          }
